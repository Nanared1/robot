...
void lift_auto(int speed, int encoder_count){
	while(ll_Encoder > -390){
		lift(50);
	}
	lift(0);
	wait1Msec(250);
...
void move_straight(int speed, int encoder_count){
	while(abs(l_Encoder) < encoder_count){
	...
	// Go other way if encoder count is greater than target
	/*while(abs(l_Encoder) > encoder_count){
		if(abs(l_Encoder) == abs(r_Encoder)){
			right_drive(-(speed));
			left_drive(-(speed));
