...
void lift_shoot(int speed, int encoder_count, int chassis_count){
    while(l_Encoder < chassis_count){
        if(abs(l_Encoder) == abs(r_Encoder)){
            right_drive(speed);
            left_drive(speed);
        }
...
task main()
{
  ...
  wait1Msec(250);
	while(true){
		encoder_lift(120, 320);
		wait1Msec(250);
		lift_shoot(120, -440, 480);
		wait1Msec(500);
		turn(1300);
		wait1Msec(500);
		  r_Encoder = 0;
