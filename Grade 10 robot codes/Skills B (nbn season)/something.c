...
#pragma config(Motor,  port10,          rightFront,    tmotorVex393HighSpeed_HBridge, openLoop)
...
task main()
{
  ...
  SensorValue[rightEncoder] = 0;	  // Set the encoder so that it starts counting at 0
	...
	while(SensorValue[rightEncoder] < 2960)		// Creates an infinite loop, since "true" always evaluates to true
	{
		{
			straight();
		}
	}
	while(abs(SensorValue[in1]) < 900 - 100)
  {
    //...Continue turning
    motor[rightFront] = -30;
    motor[rightBack] = -30;
  ...
  //Second while loop to move more slowly toward the goal
  while(abs(SensorValue[in1]) != 900)
  {
    if(abs(SensorValue[in1]) > 900)
    {
      motor[rightFront] = 30;
      motor[rightBack] = 30;
      motor[leftFront] = -30;
